...
def tampilkan_kostum(k):
    table = PrettyTable()
    table.field_names = ["ID", "Nama Kostum", "Harga Sewa"]
    table.title = "Daftar Kostum"
    for kostum in k:
        table.add_row([kostum["id"], kostum["nama"], kostum["harga"]])
    print(table)
...
def tambah_kostum(k):
    ...
    id_baru = len(k) + 1
    k.append({"id": id_baru, "nama": nama, "harga": harga})
    print("Kostum berhasil ditambahkan")

def update_kostum(k):
    tampilkan_kostum(k)
    id_kostum = int(input("Masukkan ID kostum yang ingin diupdate: "))
    for kostum in k:
        if kostum["id"] == id_kostum:
...
def hapus_kostum(k):
    tampilkan_kostum(k)
    id_kostum = int(input("Masukkan ID kostum yang ingin dihapus: "))
    for kostum in k:
        if kostum["id"] == id_kostum:
            k.remove(kostum)
...
def sewa_kostum(k):
    tampilkan_kostum(k)
    id_kostum = int(input("Masukkan ID kostum yang ingin disewa: "))
    for kostum in k:
